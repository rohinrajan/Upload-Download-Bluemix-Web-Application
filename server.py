...
def upload_or_update_file(fileobj):
    ...
    hashcontent = gethashContent(fcontent)
    print 'file name= ' +filename
    file_exists, doc_id = filename_exists(filename)
    if file_exists:
        # check to see if same content is present
        if same_hashcontent(hashcontent, doc_id):
            return "Filename= " + filename +" has already been uploaded."
        else:
            ver_id = update_file(filename,fcontent,hashcontent)
            retValue = 'The different revisions of this file are </br>'
            for _id in ver_id:
                retValue = retValue + str(_id) + '</br>'
            return retValue
    else:
        if filename.rsplit('.',1)[1] in ALLOWED_DOC:
            upload_file(filename,fcontent,hashcontent)
        else:
            upload_attachment(filename,fcontent,hashcontent)
